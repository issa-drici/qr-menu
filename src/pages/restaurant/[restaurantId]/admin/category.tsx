...
function CategoryComponent() {
    ...
    async function onSubmit() {
        ...
        try {
            ...
            const { data: dataUpdated, error } = await supabaseClient
                .from('category')
                .upsert(newCategories, { onConflict: "id", merge: ['created_at', 'name', 'order'] })
                .select()
    ...
    const handleAddCategory = () => {
        const newCategories = [...categories];
        newCategories.push({ id: v4(), order: `${categories.length + 1}`, created_at: new Date(), profile_id: user?.id, name: 'Nouvelle Catégorie', is_active: true })
                                                        ...
                                                        <div className="flex items-center max-h-14">
                                                            <CardHeader className="w-full">
                                                                {category.displayInput ?
                                                                    <Input ref={inputRef} value={category.name} onChange={e => handleChangeInput(index, e.target.value)} onBlur={() => handleEditBlur(index)} />
                                                                    :
